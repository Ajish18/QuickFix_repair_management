...
@frappe.whitelist()
def get_job_summary():
	return {"message": "Hello from QuickFix", "status": "Working"}
...
def get_overdue_jobs():
	...
	result = (
		...
		.where((JC.status.isin(["Pending Diagnosis", "In Repair"])) & (JC.creation < seven_days_ago))
...
def transfer_job(from_tech, to_tech):
	try:
		...
		frappe.db.rollback()
		frappe.log_error(message=str(e), title="Transfer Job Failed")
		raise
...
def job_ready_email(job_card_name):
	job_card = frappe.get_value(
		"Job Card", job_card_name, ["customer_email", "customer_name", "device_type"], as_dict=True
	)
	frappe.sendmail(
...
def get_status_chart_data():
	...
	data = frappe.db.get_all("Job Card", fields=["status", "count(*) as count"], group_by="status")

	# Format the data for the Frappe Chart engine
	labels = [d.get("status") for d in data]
	values = [d.get("count") for d in data]

	return {"labels": labels, "datasets": [{"name": "Job Status", "values": values}]}
...
def trigger_webhook(doc, method):
	frappe.enqueue("quickfix.api.send_webhook", job_card_name=doc.name, queue="short")
